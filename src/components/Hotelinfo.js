...
const Hotelinfo = () => {
  ...
  const loadServicesData = async() => {
    ...
    const resp = await fetch('https://w6q9elce3j.execute-api.us-east-1.amazonaws.com/Production/services');
  ...
  const loadAccessData = async() => {
    // Query the API Gateway
    const resp = await fetch('https://w6q9elce3j.execute-api.us-east-1.amazonaws.com/Production/accessibility');
    let jsonData = await resp.json();
    // Assign response data to our state variable
    setAccessData(jsonData)
  }
      ...
      <div className="scene" id="hotelinfo">
          ...
          <article id="usefulinfo">
            ...
            <section className="checklist" id="accessibility">
              ...
              <ul>
              {
                accessData.map((accessData) =>
                <li>{accessData.access}</li>
                )
              }
              </ul>
            </section>
